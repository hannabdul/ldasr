...
def get_ctc_scores(emission, index = 5):
    beam_search_result = decoder[index](emission.cpu())
    # beam_search_scores = []
    for s_ in beam_search_result:
        # print(s_[0])
        beam_search_scores = s_[0].score
        # print(beam_search_scores)
        
    return beam_search_scores
...
def avg_models(model, path, init, end, device):
    nepoch=init

    best_model=path+'/mod{:03d}-transformer-tc100'.format(nepoch)
    model.load_state_dict(torch.load(best_model,map_location=device))
    m1=model.state_dict()
    nc = 1

    # for nepoch in range(nepoch+2, end+1, 2):
    for nepoch in range(nepoch+2, end+1, 2):
        best_model=path+'/mod{:03d}-transformer-tc100'.format(nepoch)            
