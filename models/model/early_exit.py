...
class Early_encoder(nn.Module):
    ...
    def __init__(self, src_pad_idx, n_enc_replay, enc_voc_size, dec_voc_size, d_model, n_head, max_len,  dim_feed_forward, n_encoder_layers,  features_length, drop_prob, device, gate_prob = 1.0):
        super().__init__()
        self.src_pad_idx = src_pad_idx
        self.n_enc_replay=n_enc_replay
        self.device = device
        ...
        self.encoders = nn.ModuleList([Encoder(d_model=d_model,
                            ...
                            n_layers=n_encoder_layers,
                            device=device,
                            gate_prob = gate_prob) for _ in range(self.n_enc_replay)])
